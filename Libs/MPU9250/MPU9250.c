...
/// @brief Check for connection, reset IMU, and set SPI
...
uint8_t MPU_begin(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250)
{
    ...
    addr = PWR_MGMT_1;
    val = 0x00;	// Internal oscillator 20 MHz
...
void MPU_readRawData(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250)
{
    ...
    // Bit shift the data
    pMPU9250->rawData.ax = (int16_t)((buf[0] << 8) | buf[1]) - pMPU9250->accOffs.x;
    pMPU9250->rawData.ay = (int16_t)((buf[2] << 8) | buf[3]) - pMPU9250->accOffs.y;
    pMPU9250->rawData.az = (int16_t)((buf[4] << 8) | buf[5]) - pMPU9250->accOffs.z;
    pMPU9250->rawData.temp = (int16_t)((buf[6] << 8) | buf[7]);
    pMPU9250->rawData.gx = (int16_t)((buf[8]  << 8) | buf[9])  - pMPU9250->gyroOffs.x;
    pMPU9250->rawData.gy = (int16_t)((buf[10] << 8) | buf[11]) - pMPU9250->gyroOffs.y;
    pMPU9250->rawData.gz = (int16_t)((buf[12] << 8) | buf[13]) - pMPU9250->gyroOffs.z;
}
...
void MPU_calibrateGyro(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250, uint16_t numCalPoints)
{
    ...
    // Collect multiple stationary samples
    for (uint16_t i = 0; i < numCalPoints; i++)
    {
        MPU_readRawData(SPIx, pMPU9250);
        sum_x += pMPU9250->rawData.gx;
        sum_y += pMPU9250->rawData.gy;
        sum_z += pMPU9250->rawData.gz;
        osDelay(10);  // 100 Hz sampling
    }

    // Average the saved data points to find the gyroscope offset
    pMPU9250->gyroOffs.x = (int16_t)(sum_x / numCalPoints);
    pMPU9250->gyroOffs.y = (int16_t)(sum_y / numCalPoints);
    pMPU9250->gyroOffs.z = (int16_t)(sum_z / numCalPoints);
}

int16_t MPU_calibrateAcc(SPI_HandleTypeDef *SPIx, MPU9250_t *pMPU9250, uint16_t numCalPoints,
	int16_t* pDataSrc)
{
	int64_t sum = 0;
