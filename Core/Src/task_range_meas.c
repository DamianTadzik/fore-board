...
//    VL6180_WaitDeviceBooted(&tof1);
    ...
    status |= VL6180_SetXTalkCompensationRate(&tof1, 4);	// 0.038 * 128 = ~5
    status |= VL6180_SetOffsetCalibrationData(&tof1, +3);
...
//    VL6180_WaitDeviceBooted(&tof2);
    ...
    status |= VL6180_SetXTalkCompensationRate(&tof2, 5);	// 0.031 * 128 = ~4
    status |= VL6180_SetOffsetCalibrationData(&tof2, +18);
